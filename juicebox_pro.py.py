...
import pandas as pd
import numpy as np
import time  # New import for the Guard
...
def get_live_price(t):
    # RATE LIMIT GUARD: Small delay between individual stock price calls
    time.sleep(0.1) 
    try:
        tk = yf.Ticker(t)
        live_val = tk.info.get('regularMarketPrice')
        if live_val is not None: return float(live_val)
...
# -------------------------------------------------
# 3. SIDEBAR (STRICT LOGIC PRESERVATION)
# -------------------------------------------------
with st.sidebar:
    st.header("🧃 Configuration")
    acct = st.number_input("Account Value ($)", 1000, 1000000, 10000, step=500, key="cfg_acct_v29")
    goal_amt = st.number_input("Goal ($)", 1.0, 100000.0, 150.0, step=10.0)
    
    price_range = st.slider("Price Range ($)", 1, 500, (2, 100))
    dte_range = st.slider("DTE", 0, 45, (0, 30))
    ...
    cushion_val = st.slider("Min ITM Cushion %", 0, 50, 10) if is_itm else 0

    st.divider()
    etf_only = st.toggle("ETF Only Mode", value=False)
    ...
    text = st.text_area("Watchlist", value=watchlist_text, height=150)
    tickers = sorted({t.upper() for t in text.replace(",", " ").split() if t.strip()})

# -------------------------------------------------
# 4. SCANNER LOGIC (STRICT FORMULA PRESERVATION)
# -------------------------------------------------
def scan(t):
    try:
        # RATE LIMIT GUARD: Pause before hitting option chain
        time.sleep(0.2)
        tk = yf.Ticker(t)
        ...
        for exp in tk.options:
            ...
            for _, row in df.iterrows():
                ...
                # Maintain strict ATM vs ITM Logic
...
if st.button("RUN LIVE SCAN ⚡"):
    with st.spinner(f"Guard Active: Scanning watchlist with rate-limiting..."):
        # RATE LIMIT GUARD: Lower worker count for stability
