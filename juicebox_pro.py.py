...
with st.sidebar:
    ...
    st.divider()
    st.subheader("🛡️ Safety & Range")
    
    # PRECISION RANGE SELECTOR
...
# -------------------------------------------------
# 4. SCANNER ENGINE (Price Range Integrated)
# -------------------------------------------------
def scan_ticker(t, strategy_type, week_goal, cushion_limit, dte_limit, min_price, max_price):
    try:
        ...
        # Applying the user-selected price range
        ...
        for exp in stock.options[:3]:
            ...
            if 4 <= dte <= dte_limit:
                ...
                actual_prem = float(match["last_Price"])
                ...
                return {
                    "Ticker": t, "Price": round(price, 2), "Strike": strike_price,
                    "Premium ($)": round(actual_prem * 100, 2), "Juice ($)": round(juice * 100, 2), 
                    "ROI %": round((juice/basis)*100, 2), "Cushion %": round(((price - basis) / price) * 100, 2), 
                    "DTE": dte, "Contracts": contracts, "Capital Req ($)": round(capital_req, 2)
                }
    except: return None
...
if st.button("RUN GLOBAL SCAN ⚡", use_container_width=True):
    univ = ["SPY", "QQQ", "IWM", "AMD", "NVDA", "AAPL", "TSLA", "PLTR", "SOFI", "AFRM", "MARA", "RIOT", "F", "BAC", "OXY", "FAS", "SOXL"]
    ...
    if sel.selection.rows:
        ...
        with c2:
            ...
            <div class="card">
                ...
                <p style="color:#16a34a; font-size:24px; font-weight:800;">${row['Juice ($)']} Juice</p>
                <p><b>Capital Req:</b> ${row['Capital Req ($)']:,}</p>
                <hr>
                <b>Range:</b> ${min_p} - ${max_p}<br>
                <b>Actual Premium:</b> ${row['Premium ($)']}<br>
