...
# -------------------------------------------------
# 3. SIDEBAR (CONFIG & FULL WATCHLIST)
# -------------------------------------------------
with st.sidebar:
    st.header("🧃 Configuration")
    acct = st.number_input("Account Value ($)", 1000, 1000000, 10000, step=500, key="cfg_acct_v31")
    goal_amt = st.number_input("Goal ($)", 1.0, 100000.0, 150.0, step=10.0, key="cfg_goal_v31")
    
    price_range = st.slider("Price Range ($)", 1, 500, (2, 100), key="cfg_pr_v31")
    dte_range = st.slider("DTE", 0, 45, (0, 30), key="cfg_dte_v31")
    strategy = st.selectbox("Strategy", ["Deep ITM Covered Call", "Standard OTM Covered Call", "ATM Covered Call", "Cash Secured Put"], key="cfg_strat_v31")
    
    put_mode = "OTM"
    if strategy == "Cash Secured Put":
        put_mode = st.radio("Put Mode", ["OTM", "ITM"], horizontal=True, key="cfg_pm_v31")
    
    is_itm = strategy == "Deep ITM Covered Call" or (strategy == "Cash Secured Put" and put_mode == "ITM")
    cushion_val = st.slider("Min ITM Cushion %", 0, 50, 10, key="cfg_cush_v31") if is_itm else 0

    st.divider()
    etf_only = st.toggle("ETF Only Mode", value=False, key="cfg_etf_v31")
    ...
    text = st.text_area("Watchlist (200 Symbols)", value=watchlist_200, height=150, key="cfg_wl_v31")
...
if st.button("RUN LIVE SCAN ⚡"):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for i, res in enumerate(executor.map(scan, tickers)):
            if res: results_list.append(res)
            progress_bar.progress((i + 1) / len(tickers))
            status_text.text(f"Scanning {i+1}/{len(tickers)}: {tickers[i]}")
    
    st.session_state.results = results_list
    status_text.text(f"Scan Complete! Found {len(results_list)} opportunities.")
...
if "results" in st.session_state:
    ...
    if not df.empty:
        ...
        sel = st.dataframe(df[cols], use_container_width=True, hide_index=True, selection_mode="single-row", on_select="rerun", key="df_v31")
...
st.markdown("""<div class="disclaimer"><b>LEGAL NOTICE:</b> JuiceBox Pro™ owned by <b>Bucforty LLC</b>. Information for educational purposes only.</div>""", unsafe_allow_html=True)