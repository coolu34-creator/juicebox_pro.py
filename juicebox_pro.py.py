...
if st.button("RUN WEEKLY SCAN ⚡", use_container_width=True):
    univ = ["SPY", "QQQ", "IWM", "AMD", "NVDA", "AAPL", "TSLA", "PLTR", "SOFI", "AFRM", "MARA", "RIOT", "F", "BAC", "AMD"]
...
if "results" in st.session_state and st.session_state.results:
    ...
    # Selection Table
    sel = st.dataframe(df[["Ticker", "Juice ($)", "ROI %", "Contracts", "DTE"]], 
    ...
    if sel.selection.rows:
        ...
        with c1:
            st.write(f"### {row['Ticker']} Technical Chart")
            # TradingView Embed
            components.html(f"""
        ...
        with c2:
            st.markdown(f"""
            <div class="card">
                <b>{row['Ticker']} Analysis</b><br>
                <span class="juice-val">${row['Juice ($)']}</span> per contract<br>
                <hr>
                <b>Weekly Need:</b> {row['Contracts']} Contracts<br>
                <b>Expiry:</b> {row['Expiry']}<br>
                <b>DTE:</b> {row['DTE']} days
            </div>
            """, unsafe_allow_html=True)