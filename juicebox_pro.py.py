...
<style>
    ...
    .legend-box {background: #f8fafc; padding: 15px; border-radius: 10px; border: 1px solid #e2e8f0; margin-top: 10px;}
</style>
...
with st.sidebar:
    ...
    if strategy == "Standard OTM Covered Call":
        st.info("**Standard OTM:** Targets strikes above market price. You keep the 'Juice' plus stock gains up to the strike. Best for bullish stocks.")
    elif strategy == "Deep ITM Covered Call":
        st.success("**Deep ITM:** Targets strikes well below market price. Focuses on maximum safety (cushion). Best for conservative income with no stock growth.")
    elif strategy == "ATM Covered Call":
        st.warning("**ATM:** Targets strikes closest to current price. Offers high 'Juice' but very little room for the stock to move up before being called.")
    else: # CSP
        st.info("**Cash Secured Put:** You get paid to wait to buy a stock at a discount. If it stays above the strike, you keep the cash. If it drops, you buy the shares.")
...
def scan(t):
    try:
        ...
        for exp, dte in valid_exps:
            ...
            collateral_per_con = strike * 100 if is_put else price * 100
            juice_per_con = prem * 100
            contracts = max(1, int(np.ceil(goal / juice_per_con)))
            total_collateral = contracts * collateral_per_con
            ...
            if is_put:
                ...
                yield_pct = (extrinsic * 100 / collateral_per_con) * 100
            ...
            row = {
                ...
                "Juice/Con": round(juice_per_con, 2), "Contracts": contracts,
                ...
                "Collateral": round(total_collateral, 0)
            }
...
with st.expander("📖 OPERATING DIRECTIONS", expanded=False):
    st.markdown("""
    <div class="guide-box">
    <b>1. Capital Allocation:</b> Calculations respect your Account Value to ensure you aren't over-leveraged.<br>
    <b>2. Contract Calculation:</b> The scanner determines the exact number of contracts needed to hit your Weekly Goal.<br>
    <b>3. Standards:</b> OTM logic ensures you pick strikes with positive growth potential.<br>
    <b>4. Proprietary:</b> Built for Bucforty LLC with real-time volatility assessment.
    </div>
    """, unsafe_allow_html=True)
        ...
        if sel.selection.rows:
            ...
            with c2:
                ...
                <div class="card">
                    ...
                    <b>Juice per Contract:</b> ${r['Juice/Con']:,.2f}<br>
                    <b>Total Expected Juice:</b> ${r['Total Juice']:,.2f}<br>
                    <hr>
                    <b>Live Price:</b> ${r['Price']}<br>
                    <b>Strike:</b> ${r['Strike']} | <b>DTE:</b> {r['DTE']} Days<br>
...
<b>LEGAL NOTICE & DISCLAIMER:</b> JuiceBox Pro™ is a software tool owned by <b>Bucforty LLC</b>. All calculations are estimates based on 
third-party market data. Options trading involves risk of principal loss. By using this tool, you agree to hold Bucforty LLC harmless from any financial decisions or 
losses.
</div>
""", unsafe_allow_html=True)# =================================================================
# SOFTWARE LICENSE AGREEMENT
